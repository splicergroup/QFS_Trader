"""
Opening-Range Liquidity-Sweep Reversal — LONG & SHORT
======================================================
A Streamlit app that hunts for both *bullish* and *bearish* reversals across
the whole trading session after the 09:30 ET open. Same logic as the long-only
app, with short setups enabled.

Setups:
  1. The first 15-min candle must be a liquidity candle (range ≥ 25% of the
     14-day ATR) — this defines the opening-range box.
  2a. LONG  — price sweeps BELOW the box low, then a bullish reversal prints
      (Hammer / Inverted Hammer / Bullish Engulfing). Entry = break above the
      reversal high · TP = box high · 2:1 R:R.
  2b. SHORT — price sweeps ABOVE the box high, then a bearish reversal prints
      (Shooting Star / Hanging Man / Bearish Engulfing). Entry = break below
      the reversal low · TP = box low · 2:1 R:R.
  3. Optional filters: above-average volume AND the right side of VWAP.

Run:  streamlit run orb_long_short_app.py
"""
...
# Long & short, full-session configuration
ALLOW_SHORT  = True
SCAN_MINUTES = None   # no time-of-day restriction on the reversal

# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="ORB Long & Short Sweep",
    page_icon="🔀",
    layout="wide",
...
<style>
    ...
    .sidebar-title {
        ...
        color: #58a6ff; margin-bottom: 0.25rem;
    }
...
# ════════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown('<p class="sidebar-title">🔀 ORB Long & Short Sweep</p>', unsafe_allow_html=True)
    st.markdown('<p class="sidebar-sub">Bullish & bearish liquidity-sweep reversals · full session</p>',
                unsafe_allow_html=True)
    st.markdown("---")
    ...
    use_vwap = st.checkbox(
        ...
        help="Reversal candle must close on the trade's side of VWAP.",
    )
    st.caption(
    ...
    st.markdown("---")
    st.markdown("**How it works  ·  LONG & SHORT**")
    st.markdown("""
1. **ATR gate** — first 15-min candle range ≥ 25% of 14-day ATR (the box)
2. **Sweep** —
   *Long:* price dips **below** the box low (bullish reversal)
   *Short:* price pops **above** the box high (bearish reversal)
3. **Reversal** —
   *Long:* Hammer / Inv. Hammer / Bull. Engulfing
   *Short:* Shooting Star / Hanging Man / Bear. Engulfing
4. **Filters** — above-average volume **and** the right side of VWAP
5. **Entry** — break beyond the reversal candle (high for long, low for short)
6. **TP / SL** — opposite side of the box · 2:1 R:R
""")
    st.markdown("---")
    st.caption("Data via Yahoo Finance · Long & short · Not financial advice")
...
st.caption("Opening-range liquidity-sweep reversal — long and short entries, full session.")
...
if r["verdict"] in ("BUY", "SELL"):
    ...
    st.markdown(f"### Trade Levels  ·  {'LONG' if is_long else 'SHORT'}")
    m1, m2, m3, m4, m5, m6 = st.columns(6)
    m1.metric(f"Entry ({r['verdict']})", f"${r['entry']:.2f}")
...
with st.expander("📊 Full Analysis Details", expanded=False):
    ...
    with right:
        ...
        if r["signal"]:
            ...
            sc  = sig["candle"]
            is_long = r["direction"] == "long"
            heading = "Bullish Reversal" if is_long else "Bearish Reversal"
            swept = (f"below Box Low **${r['box_low']:.4f}**" if is_long
                     else f"above Box High **${r['box_high']:.4f}**")
...
# ── Backtest panel ────────────────────────────────────────────────────────────
st.markdown("---")
with st.expander("🧪 Backtest (long & short, full session)", expanded=False):
    st.caption(
        "Replays this setup day-by-day and simulates each trade (entry fill → "
        "first of TP / SL). Yahoo caps intraday history at ~60 days, so samples "
        "are small. Candle-straddle ties assume the stop fills first (conservative)."
    )
    ...
    if st.button(f"▶  Run backtest for {ticker}", use_container_width=True):
        ...
        if bt.get("error"):
            ...
            st.info(f"No filled trades for {ticker} in the window "
